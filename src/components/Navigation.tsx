  ...
  return (
    <>
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 font-mono ${
        ...
        backgroundColor: scrolled ? `var(--theme-surface)80` : 'transparent',
        borderColor: scrolled ? `var(--theme-overlay)20` : 'transparent'
      }}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    style={isActive ? { backgroundColor: `var(--theme-accent)10` } : {}}
                  >
                    {item.name}
                  </Link>
                )
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 z-30 backdrop-blur-sm transition-opacity duration-300 md:hidden"
          style={{ backgroundColor: `var(--theme-bg)50` }}
          onClick={() => setIsOpen(false)}
          aria-hidden="true"
        />
      )}
      
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-80 transform flex-col border-l shadow-2xl transition-transform duration-300 ease-in-out md:hidden ${
        ...
        {/* Sidebar Header */}
        <div className="flex h-16 flex-shrink-0 items-center justify-between border-b px-6" style={{
          borderColor: `var(--theme-overlay)20`
        }}>
          <span className="text-accent font-mono text-lg font-semibold">Navigation</span>
          <button
            onClick={() => setIsOpen(false)}
            className="p-2 rounded-lg text-subtext hover:text-red transition-colors duration-200"
        ...
        <nav className="flex-1 overflow-y-auto px-6 py-6">
          ...
          {/* Divider */}
          <hr className="my-6" style={{ borderColor: `var(--theme-overlay)20` }} />
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-3 rounded-lg text-subtext hover:text-accent transition-all duration-200"
                    style={{ backgroundColor: `var(--theme-surface)20` }}
                    aria-label={social.label}
                  >
